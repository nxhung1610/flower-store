  ...
  StatisticBloc() : super(StatisticInitial()) {
    on<StatisticLoaded>((event, emit) async {
      try {
        ...
        final _requestList =
            (await AppRepository().request.get()) as List<Bill>;
        final _invoiceList =
            (await AppRepository().invoice.get()) as List<Bill>;
        ...
        if (error is Response)
          emit(StatisticalLoadFail(message: error.data["message"]));
        else
          emit(StatisticalLoadFail(message: error.toString()));
      }
    });
